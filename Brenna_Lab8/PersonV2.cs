...
{ //public class 
    public class PersonV2 : Person   //Create new class called "PersonV2" inherit the original person class and add two new properties: Cell Phone and Facebook; Update Main 
    {
        ...
        //Create var to hold Persons ID assigned by the DB //was public
        ...
        public int Person_ID
        {
            ...
            set
            { person_ID = value; }
        } //end of public string private int Person_ID = 0;
...
//!!!!!HERE
        ...
        //It will return an Interger meant for feedback on how many records were      //was PersonV2_ID
        public string DeleteOnePerson(int intPerson_ID)
        {
            ...
            comm.Parameters.AddWithValue("@Person_ID", intPerson_ID);
                       //Make sure it is intPerson_ID not Person_ID
            try
            {
                //Open the connection
                conn.Open();
        ...
        public Int32 UpdateARecord() //change string in int32?
        {
            Int32 intRecords = 0;
            string strResult = "";
           ...
           // string strConn = GetConnected();
            ...
            //MySQL command string to pull one Person's data
            string sqlString = "UPDATE Persons SET FirstName= @FName, MiddleName= @MName, LastName= @LName, st1= @Street1, st2= @Street2, City= @City, State= @State, Zip= @Zip, Phone= @Phone, Cell= @Cell, Email= @Email, Fb= @Fb WHERE @Person_ID = Person_ID";
